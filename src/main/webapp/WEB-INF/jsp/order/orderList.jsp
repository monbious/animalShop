			...
			<table>
				<tbody>
					<c:forEach items="${info.list}" var="order">
						<tr>
							<th colspan="5">订单编号:${order.oid }&nbsp;&nbsp;&nbsp;&nbsp;订单金额（含运费15.0）:<font
					...
					<tr>
						<th colspan="5">
							<div class="pagination">
    							...
    							<input id="first" type="button" onclick="javascript:window.location.href='/order/search?id=${user.id}&pn=1';" value="首页" />
    							<input id="prev" havePrev=${info.hasPreviousPage }  type="button" value="上一页"/>
    							<c:forEach items="${info.navigatepageNums}" var="nav">
    								<input type="button" value="${nav }" onclick="javascript:window.location.href='/order/search?id=${user.id}&pn=${nav }';"/>
    							</c:forEach>
				    			<input id="next" haveNext=${info.hasNextPage }  type="button" value="下一页"/>
				    			<input id="last" type="button" onclick="javascript:window.location.href='/order/search?id=${user.id}&pn=${info.pages }';" value="末页" />
	  ...
	  $(function(){
		  ...
		  $("#prev").click(function(){
			  if($(this).attr("havePrev")){
				  window.location.href="/order/search?id=" + uid + "&pn=" + (pageNum - 1);
			  }else{
				  $("#first").trigger("click");
			  }
		  })
		  $("#next").click(function(){
			  if($(this).attr("haveNext")){
				  window.location.href="/order/search?id=" + uid + "&pn=" + (parseInt(pageNum) + 1);
			  }else{
				  $("#last").trigger("click");
			  }
		  })
			  ...
			  if(confirm("确认删除【"+oid+"】吗？")){
					...
					$.ajax({
						...
						success: function(result){
							window.location.href="/order/search?id=" + uid + "&pn=" + pageNum;
						}
					});
				}		 
		  })
